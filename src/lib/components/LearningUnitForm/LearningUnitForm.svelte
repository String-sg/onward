...
<form method="POST" novalidate use:enhance={onsubmit} class="flex flex-col gap-6">
    ...
    <div class="flex flex-col gap-4">
      ...
      >
        {#snippet children(source, index, errors)}
          <FormField label="Source Title" id="source-title-{index}" required error={errors?.title}>
            <TextInput type="text" bind:value={source.title} />
          </FormField>

          <FormField label="Source URL" id="source-url-{index}" required error={errors?.sourceURL}>
