<script lang="ts">
  ...
  // Proactively refresh presigned URLs
  $effect(() => {
    if (!onrefresh) {
      return;
    }

    // 14 mins interval (Src TTL is 15 mins)
  ...
  const handleVideoError = async () => {
    if (!video || !video.error || !onerror) {
      return;
    }
    const { code } = video.error;
  ...
  const handleLoadedMetadata = () => {
    ...
    // Resume after src refresh and restore position and play state.
    if (resumeTime !== null && (hasActivePlayback || resumeTime > 0)) {
